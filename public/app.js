...
function showPath(play) {
  ...
  $('#best').innerHTML = `<div class="big">${rp(p.fare)} · ${p.time} min</div>
    <div>${T().transfers(p.transfers)}${p.path.every(e => e.mode === 'walk' || e.dist > 0) ? ` · ${(p.dist / 1000).toFixed(1)} km` : ''} · ${T().rank(selected + 1)}${p.tags.map(t => `<span class="tag ${t}">${T()[t]}</span>`).join('')}</div>
